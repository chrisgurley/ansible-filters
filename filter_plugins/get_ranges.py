...
class FilterModule(object):
    ...
    def get_ranges(self, data, safe: bool = False, validate_input: bool = True, remove_unwanted: bool = False):
        try:
            if safe:
            ...
            if remove_unwanted:
                print('WARNING: `remove_unwanted` set to True, any item in the given list will be removed')
                original_size = len(data)
                data = [x for x in data if type(x) == int]
                print(f'Removed {original_size-len(data)} items from list')
